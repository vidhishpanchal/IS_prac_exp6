...
function SignUp({ history }) {
  ...
  const [showResendMobileOtpButton, setshowResendMobileOtpButton] = useState(false);
  const [enteredMobileOtp, setenteredMobileOtp] = useState(0);
  const [showEnterMobileOtp, setshowEnterMobileOtp] = useState(false);
  const [showVerifyMobileOtpButton, setshowVerifyMobileOtpButton] = useState(false);
  ...
  const submitHandler = async (e) => {
    ...
    if (mobileNumber.length !== 10) {
      Swal.fire("The length of Mobile Number should be 10");
    } else if (email.includes("@") === false || email.includes(".") === false) {
      Swal.fire("Enter a valid Email ID. It should contain '@' symbol");
